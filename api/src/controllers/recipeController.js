...
getRecipesByName = async (req, res) => {
    ...
    if (req.query.name) {
        try {
            ...
            const apiResult = await API.requestApi(name); /*En carpeta helpers*/
            const responseForApi = await apiResult.filter(e => e.title.includes(name));

            /*Analizamos posibles resultados*/
            /*Escenario uno: No encontro coincidencias en food DB pero si en la api*/
            if (responseForApi.length>0) {
                if (!recipes.resultsInDB) {
                    console.log('Entrando sin resultados de base');
                    recipes.resultsInDB = responseForApi;
                    res.status(200).json(recipes);
                    /*Escenario dos: Se encontraron coincidencias en food DB y en api.*/

                } else {
                    console.log('Entrando con  resultados de base');
        ...
        res.status(404).json({
            error:'Ingrese un ingrediente de la receta',
            status:404
...
getRecipesById = async (req, res) => {
    try {
        ...
        res.status(500).json({  mensaje:'Server error',error:error});
    }
